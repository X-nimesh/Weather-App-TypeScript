...
const Main_container = () => {
  const [cordinate, setcordinate] = useState(
      {'longitude':85.3240,'latitude':27.7172}
    );
  ...
  let getWeather = async () => {
      await axios({
          method: 'GET',
          url: `https://api.openweathermap.org/data/2.5/onecall?lat=${cordinate.latitude}&lon=${cordinate.longitude}&units=metric&exclude=hourly,minutely&appid=2cef435fc80892a1fcba4005dc6b4223`,
      })
  ...
  return (
    <>
      <Grid gridTemplateColumns='25% 75%' gridTemplateRows='100% 100%' w='100vw' h='100vh' >
        <GridItem bg='white'>
            <Sidebar Todayweather={TodaysWeatherData} cord={setcordinate}/>
          </GridItem>
        <GridItem bg='#fafafa' >
            <Main weeklyData={weeklyWeatherData} TodayWeather={TodaysWeatherData} />
        </GridItem>
      </Grid>
